...
while read j; do
	...
	trimal -in "$ALN_OUT".aa_ali.fasta -out "$TRIM_OUT".faa -gt 0.4 -resoverlap 0.5 -seqoverlap 50 2>> "$LOG_OUT" >> "$LOG_OUT"

	# trim nucleotide alignment
	trimal -in "$ALN_OUT".aa_ali.fasta -backtrans "$OUT_DIR""$j".fna -out "$TRIM_OUT".fna -gt 0.4 -resoverlap 0.5 -seqoverlap 50 2>> "$LOG_OUT" >> "$LOG_OUT"

	# check if after trimming the number of species is still greater than of equal to 17 (half); else, remove the trimmed file and keep track
	NUM_SP="$(grep -c ">" "$TRIM_OUT".fna)"

	echo -e $"\t"$NUM_SP survived after trimming...

	if [ "$NUM_SP" -lt 17 ]; then

		echo "$j" >> "$OUT_DIR"discarded_orthogroups_after_trimming.ls
		
		rm -f "$TRIM_OUT".{faa,fna}
