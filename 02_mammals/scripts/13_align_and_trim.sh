...
for gene in {dmrt,sox,fox}; do
	...
	if [[ $gene == sox ]] || [[ $gene == fox ]]; then
		cat 00_input/"$gene"_outgroup.faa >> 05_family_phylogeny/"$gene"_ALL.faa
	fi
	...
	clustalo -i $FASTA_out --hmm-in="$HMM" --outfmt=fa --threads=10 -o "${FASTA_out::-4}"_aligned.faa -l tmp.log --output-order=tree-order &&
